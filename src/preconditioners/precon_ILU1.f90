...
#include <messenger.h>
    ...
    !!
    !------------------------------------------------------------------------------------------------------------------
    type, extends(preconditioner_t) :: precon_ILU1_t
...
contains
    ...
    !> Initialize the ILU1 preconditioner. This is for allocating storage. In this case, we allocate
    ...
    subroutine init(self,data)
        class(precon_ILU1_t),   intent(inout)   :: self
    ...
    subroutine update(self,A,b)
        ...
        print*, ' Computing ILU1 factorization'

        !
        ! Test preconditioner initialization
        !
        if ( .not. self%initialized ) call signal(FATAL,'preconditioner::ILU1%update - preconditioner has not yet been initialized')
