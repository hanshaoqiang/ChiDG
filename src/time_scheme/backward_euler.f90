    ...
    subroutine iterate(self,data,nonlinear_solver,linear_solver,preconditioner)
        ...
        do itime = 1,self%nsteps
            ...
            if (itime == 1) then
                ...
                call write_tecio_variables(data,trim(filename),1)
            end if
            ...
            do idom = 1,data%ndomains()
                do ielem = 1,data%mesh(idom)%nelem
                    write(16,*) q%dom(idom)%lvecs(ielem)%vec(1)
                end do
            end do
            ...
            do idom = 1,data%ndomains()
                do ielem = 1,data%mesh(idom)%nelem
                    ...
                    write(14,*) b%dom(idom)%lvecs(ielem)%vec(1)
            ...
            do idom = 1,data%ndomains()
                do ielem = 1,data%mesh(idom)%nelem
                    ...
                    write(15,*) dq%dom(idom)%lvecs(ielem)%vec(1)
